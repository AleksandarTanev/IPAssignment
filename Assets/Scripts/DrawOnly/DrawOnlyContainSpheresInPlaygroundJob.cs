...
[BurstCompile]
public struct DrawOnlyContainSpheresInPlaygroundJob : IJobParallelFor
{
    [ReadOnly]
    public float speed;
    [ReadOnly]
    ...
    {
        float x = states[index].position.x;
        float y = states[index].position.y;
        float z = states[index].position.z;

        Vector3 originalDirection = states[index].velocity.normalized;
        ...
        {
            var st = states[index];
            st.velocity = adjustedDirection * speed;
            states[index] = st;
        }
    }
}